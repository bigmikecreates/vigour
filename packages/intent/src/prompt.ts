...
export function buildSystemPrompt(): string {
  ...
  return [
    ...
    "",
    "STRICT RULES:",
    "- send_message / broadcast_message: ONLY when the user explicitly says 'send', 'post', 'write to', 'message #channel'. NEVER infer send intent from conversational phrases like 'say that again', 'tell me', 'repeat', 'what did you say', 'remind me', or questions.",
    "- Do NOT invent message content. If the user did not supply the exact text to send, use unrecognized.",
    "- summarize_unread / read_mentions: for reading. Never use these to compose or send messages.",
    "",
    "Use unrecognized for anything not clearly handled: opinions, questions about Vigour itself,",
    "  conversational responses ('say that again'), jokes, 'book a flight', 'play music'.",
    "  Examples that ARE handled: 'what time is it' → query_system, 'list my desktop' → list_directory",
    '  Examples that are NOT: "say that again", "remind me what you said", "can you repeat that"',
    'If unrecognized: {"type":"unrecognized","originalQuery":"<the user\'s exact query>"}',
  ].join("\n");
}
